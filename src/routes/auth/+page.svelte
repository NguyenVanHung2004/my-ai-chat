						...
						<div id="auth-login-card" class=" sm:max-w-md my-auto pb-10 w-full dark:text-gray-100">
							...
							{#if Object.keys($config?.oauth?.providers ?? {}).length > 0}
								<div class="flex items-center justify-center w-full my-4">
									<hr class="flex-1 h-px border-0 dark:bg-gray-100/10 bg-gray-700/10" />
									{#if $config?.features.enable_login_form || $config?.features.enable_ldap || form}
										<span
											class="px-3 text-sm font-normal text-gray-900 dark:text-white bg-transparent whitespace-nowrap"
											>{$i18n.t('or')}</span
										>
									{/if}
