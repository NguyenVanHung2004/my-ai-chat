...
{#if $showSidebar}
	<div
		...
		class="h-screen max-h-[100dvh] min-h-screen select-none {$showSidebar
			? `${$mobile ? 'bg-gray-50 dark:bg-gray-950' : 'sidebar-glass'} z-50`
	...
	>
		...
		>
			...
			>
				<a
					class="flex items-center rounded-xl size-8.5 h-full justify-center hover:bg-gray-50 dark:hover:bg-white/[0.06] transition no-drag-region"
					href="/"
					draggable="false"
				...
				>
					<button
						class="flex size-[30px] justify-center items-center rounded-lg hover:bg-gray-50 dark:hover:bg-white/[0.06] transition {isWindows
							? 'cursor-pointer'
							: 'cursor-[w-resize]'}"
						on:click={() => {
							showSidebar.set(!$showSidebar);
				...
				<div
					class="{scrollTop > 0
						? 'visible'
						: 'invisible'} sidebar-bg-gradient-to-b bg-linear-to-b from-gray-50 dark:from-[#06060a]/80 to-transparent from-50% pointer-events-none absolute inset-0 -z-10 -mb-6"
				></div>
			</div>
							...
							{#if meta && isMenuItemVisible(itemId)}
								...
								>
									<a
										...
										activeMenuItemId
											? ($settings?.highContrastMode ?? false)
												? 'bg-black/[0.035] dark:bg-white/[0.08]'
												: 'bg-black/[0.035] dark:bg-white/[0.08]'
						...
						<Dropdown bind:show={showChatsMenu} align="end">
							...
							<div slot="content">
								<DropdownMenu className="min-w-[170px]">
									<button
										class="flex h-[1.6875rem] w-full items-center gap-2 rounded-xl px-2 text-[13px] select-none cursor-pointer hover:bg-gray-50/40 dark:hover:bg-white/[0.06]"
			...
			<div class="px-1 pt-1 pb-1.5 sticky bottom-0 z-10 -mt-2 sidebar">
				...
				<div class="flex flex-col pt-1.5">
					<div class="px-1 flex justify-center text-gray-700 dark:text-gray-300">
						<a
							id="sidebar-new-chat-button"
							class="group grow flex items-center space-x-2.5 rounded-xl px-3 py-2.5 hover:bg-gray-50 dark:hover:bg-white/[0.05] transition outline-none"
							href="/"
							draggable="false"
							on:click={newChatHandler}
							aria-label={$i18n.t('New Chat')}
						>
							...
							<div class="flex flex-1 self-center translate-y-[0.5px]">
								<div class=" self-center text-sm leading-5">{$i18n.t('New Chat')}</div>
							</div>
					...
					<div class="px-1 flex justify-center text-gray-700 dark:text-gray-300">
						...
						>
							...
							<div class="flex flex-1 self-center translate-y-[0.5px]">
								<div class=" self-center text-sm leading-5">{$i18n.t('Search')}</div>
							</div>
